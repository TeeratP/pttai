...
when `OPENAI_API_KEY` is unset. Set `OPENAI_API_KEY` and the exact same files call
**real OpenAI** (`gpt-5.4-nano`) — no code change.
...
| `parallelization.py` | Sectioning / voting — branches run concurrently, then join (adapted from `examples/panel.py`). | `frame → (optimist ∥ skeptic ∥ pragmatist) → verdict` |
